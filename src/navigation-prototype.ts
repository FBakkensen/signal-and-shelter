...
import {
  ...
  PRESETS,
  BOUNDS,
} from "./navigation-prototype-model.ts";
...
const sun = new THREE.DirectionalLight(0xffdfb4, 3);
sun.position.set(-10, 25, 12);
scene.add(sun);
const camera = new THREE.PerspectiveCamera(55, 1, 0.1, 180);
const framing = new ThirdPersonCamera();
framing.zoom = 0.68;
let heading = 0;
...
const torso = box(0.48, 0.8, 0.34, "#eab883");
torso.position.y = 1.05;
body.add(torso);
const head = box(0.42, 0.4, 0.4, "#f2ddbb");
head.position.y = 1.65;
body.add(head);
const visor = box(0.3, 0.12, 0.03, "#5da9b0");
visor.position.set(0, 1.65, -0.215);
body.add(visor);
for (const x of [-0.16, 0.16]) {
  const leg = box(0.2, 0.65, 0.25, "#747b89");
  leg.position.set(x, 0.325, 0);
  body.add(leg);
}
scene.add(body);
const marker = new THREE.Mesh(
  new THREE.TorusGeometry(0.35, 0.045, 6, 24),
  new THREE.MeshBasicMaterial({ color: "#ffcf8c", depthTest: false })
);
marker.rotation.x = Math.PI / 2;
marker.renderOrder = 10;
scene.add(marker);
const flag = new THREE.Mesh(
  new THREE.ConeGeometry(0.14, 0.35, 4),
  new THREE.MeshBasicMaterial({ color: "#ffcf8c", depthTest: false })
);
...
const guides: Record<Course, string> = {
  terraces:
    "Click beyond the three terraces. Compare jump timing A/B/C. Walk into a step with WASD, then release during preparation. Try taking over while airborne.",
  detour:
    "Click just beyond the low bar. Compare A and C: watch the predicted jump count and walking detour. The destination stays the same; only travel costs change.",
  frontier:
    "Click deep into the dark ground ahead. Watch the destination persist while new ground is revealed. A tall wall needs a detour; a gap near the far edge cannot be crossed.",
  clearance:
    "Click on the roof or past the low ceiling. The body cannot fit between the ledge and ceiling; it must go around. Too-high targets retain their marker at the closest reachable point.",
};
function buildWorld() {
  scene.remove(world);
  ...
  blocks.length = 0;
  const dark = box(28, 0.1, 16, "#343440");
  dark.position.set(4, 0, 0);
  world.add(dark);
  ...
  for (const solid of study.island.solids) {
    for (let x = solid.minX + 0.25; x < solid.maxX; x += 0.5) {
      for (let z = solid.minZ + 0.25; z < solid.maxZ; z += 0.5) {
        ...
        block.position.set(x, (solid.maxY + solid.minY) / 2, z);
        world.add(block);
        blocks.push({ mesh: block, x, z });
      }
    }
  }
  const terminal = box(0.5, 1, 0.5, "#80c8cd");
  terminal.position.set(study.island.terminal.x, 2.5, study.island.terminal.z);
  world.add(terminal);
  scene.add(world);
  element("guide").textContent = guides[study.course];
}
function configure() {
...
function restart() {
  ...
  const course: Course =
    selected === "detour" || selected === "frontier" || selected === "clearance"
      ? selected
      : "terraces";
  study = new NavigationStudy(course);
  keys.clear();
  configure();
  buildWorld();
}
...
element("resume-study").onclick = () => {
  study.resume();
  canvas.focus();
};
element("terminal-study").onclick = () => {
  study.openTerminal();
  keys.clear();
};
element("close-terminal").onclick = () => {
  study.closeTerminal();
  canvas.focus();
};
element("check-link").onclick = () => {
  study.linkChecked = true;
};
...
canvas.addEventListener("contextmenu", (event) => {
  ...
  const hit = raycaster.intersectObjects(
    blocks.filter((b) => b.mesh.visible).map((b) => b.mesh)
  )[0];
  const target =
    hit?.point ??
  ...
  if (target) {
    study.request({ x: target.x, z: target.z });
  }
  canvas.focus();
});
canvas.addEventListener(
  ...
  (event) => {
    event.preventDefault();
    framing.scroll(event.deltaY, event.deltaMode);
  },
  { passive: false }
);
...
function advance() {
  ...
  if (uiTime > 0.1) {
    uiTime = 0;
    element("status").textContent = study.paused
      ? "Paused"
      : `${study.phase.toUpperCase()} · ${study.message}`;
    element("stats").textContent =
      `Position ${study.position.x.toFixed(1)}, ${study.position.z.toFixed(1)} · height ${study.position.y.toFixed(2)} m · ${study.elapsed.toFixed(1)} s active · ${String(study.completedJumps)} jumps · ${String(study.recalculations)} route calculations${study.plan ? ` · plan ${study.plan.seconds.toFixed(1)} s / ${String(study.plan.jumps)} jumps` : ""}`;
    element("pause-dialog").hidden = !study.paused;
    element("terminal-dialog").hidden = !study.terminal || study.paused;
    element("link-status").textContent = study.linkChecked
...
function render() {
  ...
  for (const block of blocks) {
    block.mesh.visible = study.known(block);
  }
  marker.visible = Boolean(study.destination);
  flag.visible = marker.visible;
  if (study.destination) {
    const d = study.destination;
    const y = study.known(d)
      ? Math.max(2, study.island.heightAt(d.x, d.z)) + 0.08
      : 2.08;
    marker.position.set(d.x, y, d.z);
    flag.position.set(d.x, y + 0.6, d.z);
  }
