...
root.innerHTML = `<header><div><div class="eyebrow">Signal & Shelter · experiment</div><h1>Small steps, safe landings</h1></div><p>Try adaptive hops and automatic alignment. Compare the approach jump with a vertical-first jump at the wall.</p></header><div class="layout"><section class="world" aria-label="Traversal course"><div class="hint"><strong id="course-title"></strong>WASD move · Q/E orbit · wheel zoom · Escape pause<br>Use “Try course” to watch the same movement rules.</div><div class="bottom"><span class="badge" id="phase">Ready</span><span id="feedback">Ready</span></div></section><aside><label>Course<select id="course">${COURSES.map((c) => `<option value="${c.id}">${c.name}</option>`).join("")}</select></label><p class="description" id="description"></p><label>Actor capabilities<select id="profile">${PROFILES.map((p, i) => `<option value="${String(i)}">${p.name}</option>`).join("")}</select></label><div class="controls"><button class="primary wide" id="try">Try course</button><button id="pause">Pause</button><button id="reset">Reset</button><button id="stop">Release input</button><button id="step">One tick</button></div><label>Pause automatically at<select id="pause-at"><option value="none">Keep running</option><option value="alignment">Alignment</option><option value="preparation">Preparation</option><option value="flight">Takeoff</option><option value="recovery">Landing</option></select></label><label>Keyboard controls<select id="actor"><option value="player">Humanoid / test actor</option><option value="robot">Second actor (conflict courses)</option></select></label><button id="cross">Second actor: approach landing</button><label class="check"><input type="checkbox" id="bounds"/>Show body clearance</label><div class="phases"><span data-phase="ground"></span><span data-phase="alignment"></span><span data-phase="preparation"></span><span data-phase="flight"></span><span data-phase="recovery"></span></div><div class="stats" id="stats"></div><div id="error" role="alert"></div><details><summary>What this experiment tests</summary><p>Gold squares show required support. Blue wireframes show the separate body clearance. Appearance does not set either rule.</p><p>Early and vertical-first takeoff, lower hops, reduced flight speed, and full-area landing support use one integer simulation. Try release, direction changes, and pause during each phase.</p><p>Courses use static geometry and a few actors. Flight protection is conservative. Search failures do not prove every possible trajectory impossible. This is not a scale benchmark.</p><div class="stats" id="diagnostics"></div></details><p class="note">Throwaway prototype · human playtesting pending<br>Production gameplay remains on its normal entry.</p></aside></div>`;
