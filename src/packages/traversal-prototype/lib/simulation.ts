...
export function createTraversal(setup: Setup) {
  ...
  function step(input: State, commands: readonly Command[] = []): State {
    ...
    for (const a of orderedActors) {
      ...
      // Axis projections preserve tangential speed and discard only blocked residues.
      let moved = false;
      const x: Motion = { ...a.motion, x: intended.x, rx: intended.rx };
      if (
        validMotion(x) &&
        groundPath(a.motion, x, a.capabilities, boxes) &&
        clear(a.motion, x, a.capabilities, occupancy)
      ) {
        moved ||= x.x !== a.motion.x;
        a.motion = x;
      } else {
        a.motion.rx = 0;
      }
      const z: Motion = { ...a.motion, z: intended.z, rz: intended.rz };
      if (
        validMotion(z) &&
        groundPath(a.motion, z, a.capabilities, boxes) &&
        clear(a.motion, z, a.capabilities, occupancy)
      ) {
        moved ||= z.z !== a.motion.z;
        a.motion = z;
      } else {
        a.motion.rz = 0;
      }
