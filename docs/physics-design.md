...
### Exact contacts and simultaneous movement accepted — 2026-09-20

The user approved [Validate exact contacts and simultaneous protected movement](https://github.com/FBakkensen/signal-and-shelter/issues/36). [Archived experiment and detailed evidence](https://github.com/FBakkensen/signal-and-shelter/blob/9eab9caaf96950e1fd1ee6f3928959a039d2ac97/docs/testing/physics-contact-prototype.md) are preserved on `codex/physics-contact-prototype` at `9eab9caaf96950e1fd1ee6f3928959a039d2ac97`. The remote commit was verified and the clean return to this foundation branch completed; never merge the prototype.

Carry forward exact-fit clearance, partial integer progress/sliding, joint validation of simultaneous movement, revalidation when a leader's motion changes, and matching-time flight protection with persistent landing occupancy. Eleven focused contact tests and the 36 retained traversal tests passed with full project checks; integrated-browser controls and desktop/narrow visual inspection exercised the documented courses. The user’s approval supplies the human verdict; earlier browser timing and sustained-key limitations remain recorded in the archive.

The experiment authors a straight segment over each tick to a clipped integer endpoint, rather than a fractional-time impact followed by a velocity discontinuity. This disclosed convention has accepted experiment evidence, not proof of a complete production contact solver. Bounded local geometry, a finite trajectory family, stop fallback and explicit fixture arbitration do not establish general routing, autonomous decision policy, cross-runtime replay or 1,000-actor capacity. The final production interface, arithmetic/algorithm completeness, cache/invalidation and migration evidence obligations remain open. Robot job and movement-decision policy remain outside this wayfinder.
