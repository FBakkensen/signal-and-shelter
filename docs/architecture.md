...
### Traversal correction and reports — 2026-09-19

Human playtesting exposed a shared support-model defect on half-metre stairs: requiring the complete 0.6 m collision footprint to sit on one elevation rejected ordinary treads. Ground support now checks the actor centre independently of the unchanged full-body clearance check. The body can overhang a tread; jumps still require supported takeoff/landing, clearance, the 1 m elevation limit and continuous ground rather than gaps. Seeded-terrain and minimal staircase regressions cover this correction.

Navigation retains the last eight distinct rejected movement attempts, collapsing repeated identical attempts and recording actual planner rejection counts and the first blocked arc position. This bounded, local diagnostic history is reset with the actor. F8 or the pause menu's Copy movement report snapshots full-precision position, seed/generator version, heading, traversal phase, capabilities and that history; the UI pauses, copies the report where clipboard access succeeds and exposes selected text for manual copying. No automatic network reporting is added.
