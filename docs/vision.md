...
The user questioned decimal coordinate arithmetic and favors integer coordinates for scaling. Integer voxel/navigation coordinates, actor sub-voxel representation (floating point versus fixed-point integer units), and the performance evidence needed to choose between them remain to be settled. No scaling benchmark or integer-coordinate migration has been completed. A local centre-support change makes the reconstructed staircase cases pass, but the user disputes relaxing support; it is not an accepted resolution of the reported failures.

### Project-wide determinism and scale — 2026-09-19

The user requires deterministic, testable design across gameplay systems, not isolated fixes driven by movement reports. Hundreds or thousands of robots will use the same algorithms. Reopen the simulation design before further movement-rule changes: explicit state-transition invariants, numeric representation, command ordering, simulation time, randomness, replay and representative scale validation. Integer coordinate arithmetic is the stated preference; exact units, rounding, runtime portability and workload budgets remain design questions. This is a requirement, not a claim that the current implementation meets it.

Confirmed by the user: for the same game version, the same initial state and ordered commands must produce identical simulation state after every tick across supported browsers and machines. Visual interpolation is excluded from that guarantee. Exact supported runtime coverage and the implementation/verification approach remain to be established.

Confirmed overload behavior: while the game is actively running, insufficient processing capacity slows game time instead of skipping simulation work. Every simulation tick completes with the same rules and ordering; rendering may skip frames independently. This contract is not yet implemented or validated.

Confirmed suspension behavior: after actual browser/OS suspension, resume from the last completed simulation tick without catching up the suspended wall-clock duration. Background play continues while the runtime permits execution. Suspension produces no game progress; explicit pause remains user controlled. Implementation must distinguish suspension from ordinary overload without changing tick outcomes; the mechanism remains to be designed.

Separate platform consideration: the user is open to a standalone TypeScript application, potentially an Electron shell, if browser constraints become a demonstrated problem. This is an option to evaluate, not a decision to migrate. Keep the deterministic simulation independent of presentation and host APIs so packaging can change without replacing gameplay rules.

Confirmed architectural direction: separate the headless deterministic TypeScript simulation, presentation (rendering, animation and input), and host integration (lifecycle, storage and scheduling). Tests and benchmarks must exercise the same simulation used by the game. Retain the browser for now and evaluate desktop packaging against demonstrated constraints. These are agreed boundaries, not a claim that the current implementation fully enforces them.

Confirmed movement representation: actors may occupy and stop at positions between voxel centres, represented in smaller integer units. Movement is not restricted to transitions between voxel centres. Exact unit size, intermediate arithmetic, rounding/remainder policies and bounds must be specified and validated before implementation; this agreement does not relax collision or ground-support requirements.

Confirmed support direction, with the user's explicit condition: each actor has a support footprint (feet/base) distinct from body clearance, with its entire support footprint supported at a valid standing position. This is part of the same authoritative physics that determines where the player can move, not a separate navigation ruleset. Direct player movement, robot movement, route feasibility and jump execution must use that shared contract, parameterized by individual capabilities. The planner cannot approve a transition using looser support or collision checks. Exact support geometry remains to be specified; this supersedes the provisional centre-only support direction, which remains present in the local implementation pending redesign.

Confirmed actor interaction: the player and robots physically block one another under the shared physics; they cannot pass through each other. Navigation must account for occupied space. This expands the design requirements for representative many-actor validation; actor collision and traffic handling are not yet implemented.

Confirmed yielding and support limits: robots try to move aside for the player when a safe move exists, without pushing or automatically moving the player. If no safe move exists, movement remains blocked. Actors cannot provide standing support for the player or other robots; stacking and moving platforms are outside this first design. Yielding chooses movement requests and cannot bypass shared physics.

Confirmed robot traffic behavior: a robot may temporarily retreat or detour to let another pass while retaining its original destination. Use deterministic priority with fairness so repeated contention does not continually disadvantage the same robot where progress is feasible. If no safe resolution exists, stop and report the blockage; never overlap or teleport. This is movement coordination, separate from job selection. No guarantee of progress through physically impossible traffic configurations is implied; coordination and its validation remain unimplemented.

Confirmed scale workload: validate 1,000 simultaneously active simulated robots using repeatable production-simulation scenarios covering route requests, movement, jumps, open terrain and congestion. The user explicitly distinguishes computed robots from visible robots: simulation population and rendered population are separate workload dimensions. Vary visibility independently when measuring rendering cost; do not interpret this agreement as a requirement to render all 1,000 at once. Consistent with presentation-independent simulation, visibility must not change gameplay outcomes. Concrete performance budgets, reference hardware and supported runtime coverage remain to be established; no benchmark result is claimed.

Confirmed performance acceptance: the 1,000-active-robot workload must sustain normal game speed on the reference desktop. Correct results at substantially reduced simulation speed do not pass this target. Slowdown remains the safe overload fallback, not an acceptable substitute for meeting the benchmark. Record simulation time, memory, workload and reference hardware separately from rendering measurements. This is a target, not a measured capability of the current code.
