...
### Numeric decisions in progress — 2026-09-19

Confirmed in [Choose deterministic numeric and tick semantics](https://github.com/FBakkensen/signal-and-shelter/issues/28): 1 mm integer position units, integer region coordinates plus bounded local coordinates, and 60 simulation ticks per second independent of rendering. Preparation and recovery each last exactly 7 ticks, replacing the earlier 120 ms values. Region size, storage bounds, arithmetic/remainder rules and host pacing remain open; no production implementation or performance validation is claimed.

User-corrected numeric tuning policy: divisibility is a design constraint for authored time-based tuning, not merely a convenience. Walking speed is 4.2 m/s, exactly 70 mm per tick at 60 Hz, superseding the earlier decision to retain 4.3 m/s. Durations and periodic schedules use whole ticks. Derived quantities such as diagonal components still require explicitly defined integer arithmetic/remainder rules; divisibility of cardinal speed does not establish exact diagonal representation. Any remainder affecting future gameplay is reproducible state. Properties unrelated to time retain their natural units. This is accepted design only; production migration and performance validation remain pending.
