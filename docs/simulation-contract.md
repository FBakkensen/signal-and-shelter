...
- Actors physically block one another, cannot provide standing support, and never overlap or teleport to resolve traffic. Robots safely yield to the player without pushing or moving the player automatically. They may retreat/detour while retaining destinations; deterministic physical admission handles contention. Impossible traffic stops/reports without claiming guaranteed progress.
...
The user accepted the [migration sequence and gates](simulation-migration.md) on 2026-09-20. The foundation planning map is complete; the [implementation backlog](simulation-implementation-plan.md) now hands production work to the existing navigation execution map, with native implementation prerequisites. Verify the headless foundation, cut normal play over coherently, resume click-to-move, then complete combined replay and scale acceptance. Robot job and autonomous movement policy remain separate future work, not foundation migration prerequisites. The withdrawn implementation remains an archive that must never be merged.
