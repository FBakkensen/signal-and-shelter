...
### D015 implementation review reopened — 2026-09-19

The local centre-support change is provisional and unpushed. It makes a minimal half-metre staircase and a reconstruction using rounded screenshot coordinates pass, but does not establish how the actor reached the reported position or prove the support rule was wrong. The user explicitly challenged relaxing support and requested a project-wide deterministic, testable simulation suitable for hundreds or thousands of robots. The support contract, numeric representation and planning/execution guarantees must be reviewed before accepting this implementation. Earlier descriptions of the support change as an accepted correction are superseded by this status.
