...
3. Exercise every changed flow, including start, movement, camera, pause/resume, overview, reset, and discoveries where affected. Check that paused/overview states stop movement and that inputs release correctly.
4. Inspect screenshots for rendering defects and layout overlap. Check a narrow viewport as well as the desktop layout when UI changes. Read browser errors and resolve application failures.
