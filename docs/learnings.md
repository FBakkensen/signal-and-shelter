...
## 2026-09-19 — Withdraw traversal implementation; establish fundamentals first

- User decision: abandon the current automatic-traversal implementation and chart a shared foundation before continuing navigation and robot systems.
- Archive: `codex/automatic-keyboard-traversal` at `f09a795` preserves both implementation attempts, diagnostics, tests and the discussion. Neither passing tests nor the centre-support experiment establishes an accepted design. The active branch starts at production baseline `3929c5b`; application code is unchanged from that baseline.
- Evidence limits: the screenshot reconstruction used rounded coordinates. The original-controller diagnostic checked 138,240 steps and 472 landings (164 steered) without an unsupported grounded state; this is bounded evidence, not proof. The later exact report identified candidate rejection, not the root cause. See the [archived evidence](https://github.com/FBakkensen/signal-and-shelter/blob/f09a795/docs/testing/automatic-keyboard-traversal.md).
- Architectural findings: repeated full-solid scans and uncached candidate searches require design and measurement; no 1,000-robot benchmark exists. Deterministic replay, correctness invariants and performance acceptance are separate obligations.
- Tracking: [foundation map](https://github.com/FBakkensen/signal-and-shelter/issues/26), with [confirmed inputs and contract frontier](https://github.com/FBakkensen/signal-and-shelter/issues/27). Validation of baseline restoration is recorded in [the withdrawal check](testing/foundation-withdrawal.md).

## 2026-09-20 — Movement-conflict experiment accepted

- Initial comparison exposed a naive immediate-contact failure versus conservative flight protection. The user found that choice difficult; explaining the priority tradeoff and refining preparation behavior made the decision concrete. Failure of the naive policy does not prove all reactive alternatives impossible.
- The refined experiment lets player movement or jump preparation cancel conflicting robot preparation. Protection starts at commitment, shrinks to the remaining flight, and ends on landing; the body continues to occupy its landing. The user accepted this behavior. Vision and the [resolution](https://github.com/FBakkensen/signal-and-shelter/issues/32#issuecomment-5748276198) own the preference.
- Evidence: full checks passed 78 tests, including production-simulation trajectory equivalence, preparation yielding, committed-flight priority, occupancy, steering, pause and support cases. Integrated-browser interactions and screenshots covered the comparison, early versus late entry and ledge controls without browser warnings/errors. Brief automated key events did not establish sustained control feel.
- Geometry inspection: the visual feet span 520 mm across, versus a 600 mm clearance square and 500 mm terrain cells. This is dimensional evidence, not an approved support size or a diagnosis of prior traversal failures.
- Limits: baseline floating-point 120 Hz motion at 4.3 m/s, flat conflict fixtures, static ledges, full-authority steering, conservative spatial protection and fixed player-first order. No integer migration, general terrain traversal, fairness, automatic retries or scale claim.
- Archive: [source and detailed evidence](https://github.com/FBakkensen/signal-and-shelter/blob/c28ba49da8c538f2c781ae4db929e8b3025aaae5/docs/testing/movement-conflicts-prototype.md) on `codex/movement-conflicts-prototype`, remote commit verified before returning. No prototype code was merged or copied into the design branch.

## 2026-09-20 — Narrower humanoid stance selected

- User selected visual variant B after identifying feet wider than one terrain block as the problem. Vision owns the selected proportions. The user explicitly clarified that physics defines possibilities independently of appearance; alignment is desired in this case, not a mesh-derived rule.
- Shared the actual avatar factory and distance-based leg pose between normal rendering and the visual study; separate authored support outlines exposed the distinction. Facing, pose, shelf width and offset are inspection controls, not simulated movement.
- Evidence: 71 tests and all project checks passed; integrated-browser variant/pose/shelf controls, desktop and narrow screenshots, and normal-entry avatar rendering were checked. Browser warnings/errors were empty. Top-view transparency and narrow text overlap were corrected. No narrow-ridge traversal, integer-physics, support-shape or performance claim.
- [Archived comparison](https://github.com/FBakkensen/signal-and-shelter/blob/a03f661feb780137fd5c4a05ac700fb1f5c9d417/docs/testing/feet-stance-prototype.md): `codex/feet-stance-prototype` at `a03f661feb780137fd5c4a05ac700fb1f5c9d417`, remote verified before returning to the design branch. Never merge. Physical support dimensions/shape remain open in the shared-physics decision.

## 2026-09-20 — Adaptive integer traversal experiment accepted

- User verdict: “this is working as expected.” The [decision](https://github.com/FBakkensen/signal-and-shelter/issues/34) owns acceptance; [archived evidence](https://github.com/FBakkensen/signal-and-shelter/blob/473279eba7f809bb17c34757be8e12f724dbbaf1/docs/testing/integer-traversal-prototype.md) records exact courses, arithmetic conventions, browser observations and limits. Archive `codex/integer-traversal-prototype` at `473279eba7f809bb17c34757be8e12f724dbbaf1` was pushed and verified before returning to the foundation branch. No source was merged or copied into production.
- The new headless integer transition was shared by trials and execution, with full-area support and swept clearance. A bounds audit exposed a predictable invalid continuation near the coordinate limit: trial validation now rejects the future out-of-range apex before commitment, using the same motion predicate as committed-state validation. Preserve this regression in eventual production work.
- Evidence: full project checks passed; the archive commit hook passed all 105 tests, including 36 focused traversal tests and one host-input test. Integrated-browser checks covered course/profile controls, vertical-first movement, supported landings, pause/single stepping, blockage and desktop/narrow visuals; final browser logs were empty. Brief automated keyboard events and UI latency did not establish sustained input feel or precise conflict timing; the later user verdict supplies human acceptance without changing those automation limits.
- Limits: bounded local geometry, finite target/trajectory candidates, limited actors and conservative spatial protection. Single-course timings are debugging measurements, not a capacity claim. Region migration, cross-runtime replay, general fairness and normal-speed scale gates remain open.

## 2026-09-20 — Exact contacts and simultaneous movement accepted

- User verdict: “approved.” [Resolution ticket](https://github.com/FBakkensen/signal-and-shelter/issues/36); [archived source and evidence](https://github.com/FBakkensen/signal-and-shelter/blob/9eab9caaf96950e1fd1ee6f3928959a039d2ac97/docs/testing/physics-contact-prototype.md).
- The archived integer kernel already handled exact touching and full-area support, but rejected partial ground steps and used sequential actor mutation plus untimed continuation envelopes. The experiment reused its geometry/arithmetic and replaced those mechanisms on an isolated branch.
- A new regression exposed dependency handling that stopped a clipped leader unnecessarily. Checking compatible reductions before stop fallback allowed both leader and follower to advance 45 mm together. A separate timed-flight test demonstrates overlapping spatial paths at different times with safe distinct landings; a union of all future space would reject them.
- Full project checks passed, including 11 new contact cases and 36 retained traversal cases. Integrated-browser controls confirmed exact-fit versus 599 mm blockage, joint 45 mm stops, following, timed landings, pause/single-step and post-landing occupancy. Desktop/narrow screenshots and browser logs were inspected. Archive commit hooks also passed types/tests.
- Limits: straight tick segments to clipped integer endpoints; bounded actor count/local geometry and candidate family; explicit fixture arbitration; no autonomous job/replanning policy, cross-runtime replay or scale evidence. Browser action latency did not establish precise in-flight input timing. Human approval does not erase those limits.
- Archive `codex/physics-contact-prototype` at `9eab9caaf96950e1fd1ee6f3928959a039d2ac97` was pushed and verified before returning cleanly to `codex/deterministic-simulation-foundation`. No prototype source was merged or copied into the design branch.

## 2026-09-20 — Finish the physics decision after its evidence

- Repeatedly closing supporting experiments while retaining vague “remaining engineering obligations” on the parent created a moving finish line. The user explicitly requested completing the contract. Replaced the accumulated working interview with one consolidated physics design and concrete implementation gates owned by the existing migration checkpoint.
