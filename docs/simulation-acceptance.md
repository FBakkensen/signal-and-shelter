...
A separate burst issues destinations to all 1,000 actors on one tick. Deterministic queuing is allowed, but each initial request must receive a valid route result within **300 ticks (five simulation seconds)** in the fixed benchmark scenarios. Do not supersede those requests before measuring completion. A result may establish unreachability under the accepted routing contract; enqueue acknowledgement, cancellation and fabricated failure do not count. `no-candidate-in-family` is an incomplete search outcome, not a valid route result or proof of unreachability, and does not satisfy this deadline. Report it separately; the request must still receive a valid result by tick 300 or the burst gate fails. This is a route-result deadline, not an arrival guarantee. Continue normal simulation and controls while planning; the existing frame and normal-speed gates still apply to the burst run.

The [physics contract](physics-design.md) defines deterministic logical work accounting and safe movement admission. The migration checkpoint selects versioned numerical quotas and measures them against these gates; route completion cannot depend on wall-clock timing or worker arrival order. Autonomous job/movement policy remains outside this map. Congestion fixtures must disclose scripted yielding/retreat intents and exercise production safety and active movement; they do not establish autonomous fairness. The checkpoint must reconcile that policy scope explicitly without silently relaxing any workload or target.
...
- Safe contention/yielding, no overlap or actor-provided support, no player pushing or teleporting, and valid impossible-traffic outcomes. Scripted yielding checks establish physical safety only; autonomous fairness belongs to the separate job/movement decision work.
...
Publish the commit, configuration, fixture versions, raw per-run measurements, replay comparison results, interaction evidence and unresolved failures in `docs/testing/`. The consolidated [physics contract](physics-design.md) supplies the invariants for those tests. The foundation migration checkpoint must sequence implementation and evidence collection. Failure requires diagnosis and a new explicit decision if a target must change; no silent relaxation of correctness, workload or performance thresholds.
